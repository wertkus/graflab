...
void OnUpdate()
{
	...
	if (GetAsyncKeyState('W') & 0x8000)
	{
		...
		// Duvarlarla kesişim testi
		float tWalls = IntersectTriangle(Ro, Rd, vertices_Walls, vertexCount_Walls, XMMatrixIdentity());

		bool canMove = true;
		if (tWalls > 0.0f && tWalls < 7.0f)
			canMove = false;
	...
	if (!(GetAsyncKeyState(VK_RBUTTON) & 0x8000))		// Right Button is UP (for Zoom out)
	{
		...
		g_Projection = XMMatrixPerspectiveFovLH(XM_PIDIV4,m_width / (FLOAT)m_height, 0.01f , 1000.0f);
	...
	if (GetAsyncKeyState(VK_RBUTTON) & 0x8000)		// Right Button is PRESSED (for Zoom in)
	{
		...
		g_Projection = XMMatrixPerspectiveFovLH(XM_PI / 12.0f, m_width / (FLOAT)m_height, 0.01f, 1000.0f );
	...
	float TankMissileRotationAngle = 0.0f; // açı için global değişken

	if (TraceTankMissile)
	{
		XMStoreFloat4x4(&g_World_Missile_4x4, g_World_Missile);

		g_World_Missile_4x4._41 += 0.1f * Rd_Tank_Missile_Float4.x;
		g_World_Missile_4x4._42 += 0.1f * Rd_Tank_Missile_Float4.y;
		g_World_Missile_4x4._43 += 0.1f * Rd_Tank_Missile_Float4.z;

		g_World_Missile = XMLoadFloat4x4(&g_World_Missile_4x4);

		TankMissileRotationAngle += 0.3f;
		XMMATRIX mRotateMissile = XMMatrixRotationZ(TankMissileRotationAngle);
		g_World_Missile = mRotateMissile * g_World_Missile;
	}
	...
	if (intersections.size() > 0)
	{
		...
		if (nearest.isWall && (Missile_RedDot_Distance < 0.5f))
		{
			...
			XMMATRIX mScale = XMMatrixScaling(1 + RedDot_Distance * 0.03f, 1 + RedDot_Distance * 0.03f, 1 + RedDot_Distance * 0.03f);
			g_World_RedDot = mScale * RedDot_Translation;

			
		}
		else if (nearest.isEnemy && (Missile_RedDot_Distance < 0.5f))
		{
			// düşman vurma
			if (PlayTankHitSoundOnce) { PlaySound(TEXT("hit.wav"), NULL, SND_FILENAME | SND_ASYNC); PlayTankHitSoundOnce = false; }
			renderEnemy = false;
			renderTankMissile = false;
			if (FireEnemyMissile) renderEnemyMissile = false;
